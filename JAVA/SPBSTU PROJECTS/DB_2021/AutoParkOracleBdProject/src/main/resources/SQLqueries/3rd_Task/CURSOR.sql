CREATE OR REPLACE PROCEDURE reward(from1 in TIMESTAMP,
    ...
    CURSOR PERSONNEL
        IS
        SELECT COUNT(PERSONNEL_ID) as ccc, PERSONNEL_ID
        FROM (SELECT ROUTE_ID, MIN(TIME_IN - TIME_OUT) tutu
              FROM JOURNAL
                       JOIN AUTO A2 on JOURNAL.AUTO_ID = A2.ID
              where TIME_IN is not NULL
                AND TIME_OUT BETWEEN from1 AND to2
              GROUP BY ROUTE_ID)
                 JOIN
...
DECLARE
    ...
    reward3     float;
begin
    reward(TIMESTAMP '2018-04-03 23:57:03.000', TIMESTAMP '2023-04-03 23:57:03.000',125,Id1,Id2,Id3,reward1,reward2,reward3);
