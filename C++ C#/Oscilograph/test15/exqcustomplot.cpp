...
//Ex Anton Kushnir
void QCPAxis::EXrescale(bool onlyVisiblePlottables, int addPersent, bool extendToZero)
{
    //TODO: log scale!
    uint UaddPersent = qAbs(addPersent);
    QCPRange newRange;
    double Size;
    double newLower;
    double newUpper;
    rescale(onlyVisiblePlottables);
    if(axisType()==atBottom||axisType()==atTop) return;//TODO
    Size=range().size();
    newLower=range().lower-Size*(static_cast<double>(UaddPersent)/100);
    newUpper=range().upper+Size*(static_cast<double>(UaddPersent)/100);
//    qDebug()<<"newLower="<<newLower; //TODO
//    qDebug()<<"newUpper="<<newUpper;

    QCPAbstractPlottable::SignDomain signDomain = QCPAbstractPlottable::sdBoth;
    if (newUpper<0) signDomain=QCPAbstractPlottable::sdNegative;
    if (newLower>0) signDomain=QCPAbstractPlottable::sdPositive;
    if (!(extendToZero)||signDomain==QCPAbstractPlottable::sdBoth)
        newRange=QCPRange(newLower,newUpper);
    else if (signDomain==QCPAbstractPlottable::sdNegative)
        newRange=QCPRange(newLower,0);
    else if (signDomain==QCPAbstractPlottable::sdPositive)
        newRange=QCPRange(0,newUpper);
    setRange(newRange);
}
//
...
void QCustomPlot::EXrescaleAxes(bool onlyVisiblePlottables, int addPersent,
                          bool extendToZero)
{
    QList<QCPAxis*> allAxes;
    foreach (QCPAxisRect *rect, axisRects())
    ...
    foreach (QCPAxis *axis, allAxes)
      axis->EXrescale(onlyVisiblePlottables);
}
...
void QCustomPlot::enterEvent(QEvent* event)//EX Anton Kushnir
{
    QWidget::enterEvent(event);
    emit mouseEnter(event);
}

void QCustomPlot::leaveEvent(QEvent* event)//EX Anton Kushnir
{
    ...
    if (mRangeDrag.testFlag(Qt::Horizontal))
    {
      if (QCPAxis *rangeDragHorzAxis = mRangeDragHorzAxis.data())
      {
        if (rangeDragHorzAxis->mScaleType == QCPAxis::stLinear)
        {
          ...
          //EX Anton Kushnir
//          if (mIsMaxRanges==true)
//          {
//            if (mDragStartHorzRange.lower+diff<mHorizontMaxRange.lower)
//                diff=mHorizontMaxRange.lower-mDragStartHorzRange.lower;
//            else if (mDragStartHorzRange.upper+diff>mHorizontMaxRange.upper)
//                diff=mHorizontMaxRange.upper-mDragStartHorzRange.upper;
//          }
...
//          if (mIsMaxRanges==true)
//          {
//            if (mDragStartHorzRange.lower+diff<mVertMaxRange.lower)
//                diff=mDragStartHorzRange.lower-mVertMaxRange.lower;
//            else if (mDragStartHorzRange.upper+diff>mVertMaxRange.upper)
//                diff=mVertMaxRange.upper-mDragStartHorzRange.upper;
//          }
...
//EX Anton Kushnir
//Sorry, this is extremely ugly! Anton Kushnir
//void QCPAxisRect::EXAutoSetMaxRanges()
//{
//    QCPRange oldXrange = parentPlot()->xAxis->range();
//    QCPRange oldYrange = parentPlot()->yAxis->range();
//    parentPlot()->EXrescaleAxes();
//    setHorizontMaxRange(parentPlot()->xAxis->range());
//    setVertMaxRange(parentPlot()->yAxis->range());
//    parentPlot()->xAxis->setRange(oldXrange);
//    parentPlot()->yAxis->setRange(oldYrange);
//}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void QCPItemTracer::EXsetGraphKey(double key)
{
  mGraphKey = key;
  updatePosition();
}
